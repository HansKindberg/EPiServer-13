...
public abstract class PageControllerBase<T> : PageController<T>, IModifyLayout where T : SitePageData
{
	...
	{
		await this.UISignInManager.Service.SignOutAsync();
		return this.Redirect(this.HttpContext.RequestServices.GetService<UrlResolver>().GetUrl(this.PageContext.ContentLink, this.PageContext.LanguageID));
	}

	public virtual void ModifyLayout(LayoutModel layoutModel)
	{
		if(this.PageContext.Content is SitePageData page)
		{
